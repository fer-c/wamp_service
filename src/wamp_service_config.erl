...
%% @end
%% =============================================================================
-module(wamp_service_config).
-behaviour(app_config).

-include_lib("kernel/include/logger.hrl").

-define(APP, wamp_service).
...
init() ->
    ...
    _ = ?LOG_INFO(#{message => "WAMP Service configuration initialised"}),
    ok.
...
%% @private
setup_dependencies() ->
    _ = case application:get_env(kernel, applications) of
        {ok, Apps} when is_list(Apps) ->
            [logger:set_application_level(App, Level) || {App, Level} <- Apps];
        undefined ->
            ok;
        {ok, _} ->
            ok
